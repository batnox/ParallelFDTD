...
{    
        ...
        // int x0=(i0-1)*ds;
        int i,j;
        #pragma omp parallel for private(i,x,sine,sine1,j,omp2x,betax,const5x,const6x,extk,omp2y,betay,const6y,const5y,eytk) schedule(static)
        for(ii=0;ii<nx;ii+=itile){
            for(i=ii;i<ii+itile;i++){
				...
				/*__assume_aligned(den, 64);
				__assume_aligned(exs, 64);
				__assume_aligned(eys, 64);
				__assume_aligned(exi, 64);
				__assume_aligned(exi1, 64);
				__assume_aligned(eyi, 64);
				__assume_aligned(eyi1, 64);
				__assume_aligned(vx, 64);
				__assume_aligned(vy, 64);
				__assume_aligned(ext, 64);
				__assume_aligned(eyt, 64);
				__assume_aligned(hzs, 64);*/
                #pragma simd aligned
                for(j=0;j<ny;j++){
...
void anim(struct timeval total_start, FILE *file_time){
    ...
    void ELEC_DENS(){
        ...
        _mm_free diffusion + ambipolar dIFfusion + ioniZation
        */
