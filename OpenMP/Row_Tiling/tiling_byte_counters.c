...
jmid = malloc(sizeof(int) * 6200);
    ...
    x0=(i0-1)*ds;
        ...
        if(TIMD>tstop){
            ...
            printf("Bytes Transfered: float: %lld, double: %lld, integer: %lld\n", fc,dc,ic); 
            free_all();
            exit(0);
        }
        // INC_EFIELD(); 
...
{    
        ...
        //   i0=2;
        // int x0=(i0-1)*ds;
        #pragma omp parallel for private(i,x,sine,sine1,j,omp2x,betax,const5x,const6x,extk,omp2y,betay,const6y,const5y,eytk) schedule(dynamic)
